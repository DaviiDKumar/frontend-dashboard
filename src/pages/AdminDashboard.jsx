    ...
    <div className="bg-[#f8fafc] min-h-screen text-slate-900 font-sans pb-20">
      ...
      <div className="p-4 md:p-10 max-w-7xl mx-auto space-y-10">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                System Active • {currentUser?.username || "Admin"}
              </p>
            </div>
          </div>

          <div className="flex gap-4">
             <div className="bg-white px-6 py-4 rounded-3xl border border-slate-100 shadow-sm text-center min-w-[140px]">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Total Agents</p>
                <p className="text-2xl font-black">{users.filter(u => u.role === 'user').length}</p>
             </div>
             <div className="bg-blue-600 px-6 py-4 rounded-3xl shadow-lg shadow-blue-200 text-center min-w-[140px]">
                <p className="text-[10px] font-bold text-blue-100 uppercase tracking-widest leading-none mb-1">Total Leads</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-black">Upload Leads</h3>
                <span className="text-[10px] bg-blue-50 text-blue-600 font-bold px-3 py-1 rounded-full uppercase">Excel Sync</span>
            </div>

            <div className="group relative border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center hover:border-blue-400 hover:bg-blue-50/30 transition-all flex flex-col items-center justify-center">
                <FolderIcon />
                <input
                    type="file"
                    accept=".xlsx, .xls, .csv"
                ...
                <p className="text-sm font-bold text-slate-500 max-w-[200px] leading-relaxed">
                    {file ? <span className="text-blue-600 font-black">{file.name}</span> : "Drop your Excel leads here or click to upload"}
                </p>
            </div>
            ...
            <button
              ...
              className="mt-6 w-full flex justify-center items-center bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-blue-100 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <UploadIcon />
              {loading ? "Uploading..." : "Upload Leads Now"}
            </button>

            {message && (
              <div className="mt-4 p-3 bg-green-50 rounded-xl flex items-center gap-2 border border-green-100">
                <div className="h-1.5 w-1.5 rounded-full bg-green-500"></div>
                <p className="text-[10px] font-bold text-green-700 uppercase">{message}</p>
              </div>
            )}
          </section>

          {/* Create User Section */}
          <section className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-black">Create User</h3>
                <span className="text-[10px] bg-slate-100 text-slate-600 font-bold px-3 py-1 rounded-full uppercase">Agent Provisioning</span>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Secure Password</label>
                    <input
                    type="password"
                    placeholder="••••••••"
                    value={password}
            ...
            >
              <UserPlusIcon />
              {loading ? "Creating..." : "Create User Now"}
            </button>
          </section>
        </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map((u) => (
              <div key={u._id} className="group bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                <div className={`absolute top-0 right-0 w-2 h-full ${u.role === 'admin' ? 'bg-purple-500' : 'bg-blue-500'}`}></div>
                ...
                <h4 className="font-black text-lg truncate text-slate-800">{u.username || "Anonymous Agent"}</h4>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 flex justify-between items-center mb-4 border border-slate-100/50">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Total Leads</p>
                ...
                {u.role !== "admin" && (
                  <button
                    ...
                    className="w-full flex items-center justify-center text-[10px] uppercase font-black text-red-400 py-3 rounded-xl hover:bg-red-50 hover:text-red-600 transition-colors border border-transparent hover:border-red-100"
                  >
                    <TrashIcon />
                    Revoke Access
                  </button>
